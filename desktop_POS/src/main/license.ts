...
function verifyEd25519(pubKey: Buffer, data: Buffer, sig: Buffer): boolean {
  try {
    const keyObject = createPublicKey({ key: pubKey, format: 'der', type: 'spki' })
    return verifySig(null, data, keyObject, sig)
  } catch {
    return false
  }
}
...
export function isOperational(): boolean {
  ...
  // Allowed: valid license, grace period, or an active (non-elapsed) trial.
  // Blocked: elapsed trial (daysRemaining <= 0) and hard expiry.
  if (status.state === 'trial') return status.daysRemaining > 0
  return status.state === 'valid' || status.state === 'grace'
}

const LICENSE_BLOCKED_ERROR = {
  ok: false,
  error: 'LICENSE_REQUIRED',
  licenseRequired: true
} as const

export function licenseBlockedError(): typeof LICENSE_BLOCKED_ERROR {
  return LICENSE_BLOCKED_ERROR
}
...
export function getLicenseStatus(): LicenseStatus {
  ...
  if (!key || !expiryStr) {
    const trialStartStr = getSetting('trialStart')
    const trialStart = trialStartStr ? new Date(trialStartStr) : new Date()
    const trialEnd = new Date(trialStart)
    trialEnd.setDate(trialEnd.getDate() + TRIAL_PERIOD_DAYS)
    const now = new Date()
    if (now >= trialEnd) {
      return { state: 'trial', daysRemaining: 0, expiry: trialEnd.toISOString() }
    }
    const daysRemaining = Math.ceil((trialEnd.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
    return { state: 'trial', daysRemaining, expiry: trialEnd.toISOString() }
  }
